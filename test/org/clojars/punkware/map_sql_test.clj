...
(testing "map-sql"
  (deftest filter-records
    (is (= (set? (from db where :name "bar"))
           true)
        "where should return a set.")
    
    (is (= (set? (from db where-contains :name "ba"))
           true)
        "where-contains should return a set.")
        
    (is (= (get (first (from db where :account "bar-account")) :code)
           "bar-code")
        "a first record with :code equals to \"bar-code\" should exist.")
    
    (is (= (get (first (from db where :account "foo")) :account)
          nil)
        "a first record with :account equals to \"foo\" should not exist.")
    
    (is (= (get (first (from db where :foo "bar-account")) :code)
          nil)
        "a first record with key :foo should not exist.")
    
    (is (= (get (first (from db where-contains :account "bar")) :account)
          "bar-account")
        "a first record with key :foo should not exist.")
    
    (is (= (empty? (from db where :account "foo"))
          true)
        "there should be no records where :account is equal to \"foo\".")
    
    (is (= (empty? (from db where :account))
          true)
        "there should be no records when :account is not defined.")
    
    (is (= (get (first (from db where)) :code)
          "bar-code")
        "when 'where' is called without variadic arguments, the whole database should be returned.")
    
    (is (= (count (where (atom #{{:name "bar" :account "bar-account" :code "bar-code"} {:name "foo" :account "foo-account" :code "foo-code"}}) :name "bar" :code "bar-code"))
          1)
        "when a record matches several keys-values, the record should be returned only once."))
  
  
  (deftest ordered-records
    (is (= (:code (last (order-by #{{:name "foo1" :account "foo-account1" :code "foo-code1"} {:name "foo2" :account "foo-account2" :code "foo-code2"} {:name "foo1" :account "foo-account2" :code "foo-code3"}} :code)))
          "foo-code3")
        "records sould be ordered by :code ascending")
    
    (is (= (:code (last (order-by #{{:name "foo1" :account "foo-account1" :code "foo-code1"} {:name "foo2" :account "foo-account2" :code "foo-code2"} {:name "foo1" :account "foo-account2" :code "foo-code3"}} :name)))
          "foo-code2")
        "records sould be ordered by :name ascending")
    
    (is (= (:code (last (order-by #{{:name "foo1" :account "foo-account2" :code "foo-code1"} {:name "foo1" :account "foo-account1" :code "foo-code3"}} :name :account)))
          "foo-code1")
        "records sould be ordered by :name :account ascending"))
  
  
  (deftest select-records
    (is (= (select from (atom #{{:a 1, :c 2, :b 3} {:a 3, :c 1, :b 2} {:a 1, :c 3, :b 1}}))
          #{{:a 1, :c 2, :b 3} {:a 3, :c 1, :b 2} {:a 1, :c 3, :b 1}})
        "should reurn the whole database.")
    
    (is (= (select from (atom #{{:a 1, :c 2, :b 3} {:a 3, :c 1, :b 2} {:a 1, :c 3, :b 1}}) where :b 1 )
          #{{:a 1, :c 3, :b 1}})
        "should return only record where b = 1.")
    
    (is (= (select from (atom #{{:a 1, :c 2, :b 3} {:a 3, :c 1, :b 2} {:a 1, :c 3, :b 1}}) where :a 1 )
          #{{:a 1, :c 3, :b 1} {:a 1, :c 2, :b 3}})
        "should return only records where a = 1.")
    
    (is (= (sort (distinct (apply concat (map keys (select :b :c from (atom #{{:a 1, :c 2, :b 3} {:a 3, :c 1, :b 2} {:a 1, :c 3, :b 1}}))))))
          [:b :c])
        "should return only :b & :c keys.")
    
    (is (= (:b (last (select :b :c from (atom #{{:a 1, :c 2, :b 3} {:a 3, :c 1, :b 2} {:a 1, :c 3, :b 1}}) where :b 1 :a 1 order-by :b)))
          3)
  ...
  (deftest insert-record
    ...
    (is (= (count (in db insert :name "foo" :account "foo-account" :code "foo-code"))
          2)
  ...
  (deftest update-records
    ...
    (is (= (get (first (in db where :name "foo" update :code "mynewcode")) :code)
          "bar-code")
        "after trying to update a record that doesn't exist, the old value should still be there.")
    
    (is (= (get (first (in db where :name "bar" update :code "mynewcode")) :code)
          "mynewcode")
  ...
  (deftest delete-keys
    ...
    (is (= (get (first (in db where :name "bar" delete-key :code2)) :code)
          "bar-code")
        "after trying to delete an unexisting key in the first record, :code should still be there.")
    
    (is (= (get (first (in db where :name "bar" delete-key :code)) :code)
          nil)
  ...
  (deftest rename-keys
    ...
    (is (= (get (first (in db where :name "bar" rename-key :test :fail)) :fail)
          nil)
        "after trying to rename a key that doesn't exist in the first record, the renamed key should not exist.")
    
    (is (= (get (first (in db where :name "bar" rename-key :code :password)) :code)
          nil)
        "after renaming a key in the first record, :code should not exist anymore.")
    
    (is (= (get (first (in db where :name "bar" rename-key :code :password)) :password)
          "bar-code")
